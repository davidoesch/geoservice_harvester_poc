...
def scrape(source,service,i,layertree, group,layer_data,prefix):
    ...
    if "WMS" in type:
        ...
        layer_data["ABSTRACT"]= ("" if service.identification.abstract is None else service.identification.abstract)+" "+("" if service.contents[i].abstract is None else service.contents[i].abstract)
        layer_data["KEYWORDS"]= ", ".join(service.contents[i].keywords+service.identification.keywords)
